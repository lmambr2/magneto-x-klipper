#!/usr/bin/env python3
"""Guard Magneto X assets against silent loss during upstream syncs.

Works on both tracks:
  - magneto-x          (base: Klipper3d)
  - magneto-x-kalico   (base: Kalico)

Checks magneto/MANIFEST.json:
  - owned files exist and contain required symbols
  - base_files (provided by the host fork, e.g. Kalico gcode_shell_command) exist
...
def _check_file_entries(
    repo_root: Path, entries: list, label: str, errors: list[str]
) -> None:
    ...
    for entry in entries:
        rel = entry["path"]
        if not exists(rel):
            errors.append(f"missing {label} file: {rel}")
            continue
        body = text_of(rel)
...
def check_manifest(repo_root: Path, manifest: dict | None = None) -> list[str]:
    ...
    _check_file_entries(
        repo_root, manifest.get("owned_files", []), "owned", errors
    )
    # base_files: provided by Kalico/mainline host; same presence rules as owned
    _check_file_entries(
        repo_root, manifest.get("base_files", []), "base", errors
    )
    ...
    for entry in manifest.get("owned_files", []) + manifest.get(
        "base_files", []
    ):
        rel = entry["path"]
        if rel.endswith(".py"):
            compile_targets.add(rel)
    for rel in sorted(compile_targets):
        p = repo_root / rel
        if not p.is_file():
            continue
        try:
